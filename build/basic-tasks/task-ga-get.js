"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = y[op[0] & 2 ? "return" : op[0] ? "throw" : "next"]) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [0, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var googleapis_1 = require("googleapis");
var task_base_1 = require("../task-base");
var analytics = googleapis_1.google.analytics('v3');
/* tslint:disable:no-any */
var TaskGAGet = /** @class */ (function (_super) {
    __extends(TaskGAGet, _super);
    function TaskGAGet(connections) {
        var _this = _super.call(this) || this;
        _this.connections = connections;
        return _this;
    }
    TaskGAGet.prototype.execute = function (routeMatch, routeTaskConfig, renderer) {
        return __awaiter(this, void 0, void 0, function () {
            var config, connection, scope, jwtClient, credentials, parameters, template, rendered, options, rows;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (!renderer) {
                            throw new Error('No renderer specified');
                        }
                        config = routeTaskConfig.config;
                        connection = this.connections[config.connection];
                        routeMatch.log('Setting up new JWT Client');
                        scope = ['https://www.googleapis.com/auth/analytics.readonly'];
                        jwtClient = new googleapis_1.google.auth.JWT(connection.clientEmail, undefined, connection.privateKey, scope, undefined);
                        routeMatch.log('Authorising JWT Client');
                        return [4 /*yield*/, jwtClient.authorize()];
                    case 1:
                        credentials = _a.sent();
                        routeMatch.log('Setting credentials', credentials);
                        jwtClient.setCredentials(credentials);
                        googleapis_1.google.options({ auth: jwtClient });
                        if (!(typeof config.parameters === 'string')) return [3 /*break*/, 3];
                        routeMatch.log('Template passed as a template so rendering it');
                        template = routeMatch.getString(config.parameters);
                        return [4 /*yield*/, renderer.render(template, routeMatch)];
                    case 2:
                        rendered = _a.sent();
                        if (typeof rendered === 'string') {
                            routeMatch.log('Renderer responsed with string so parsing it');
                            parameters = JSON.parse(rendered);
                        }
                        else {
                            parameters = rendered;
                        }
                        return [3 /*break*/, 4];
                    case 3:
                        parameters = config.parameters;
                        _a.label = 4;
                    case 4:
                        routeMatch.log('Parameters have been compiled into the following:', parameters);
                        options = { auth: jwtClient };
                        routeMatch.log('Getting data using connection details:', options);
                        return [4 /*yield*/, this.getData(parameters, options, routeMatch)];
                    case 5:
                        rows = _a.sent();
                        routeMatch.log('Got data.', rows.length, 'rows');
                        routeMatch.setData(rows);
                        return [2 /*return*/, { command: task_base_1.TaskResultCommand.CONTINUE }];
                }
            });
        });
    };
    TaskGAGet.prototype.getData = function (params, options, routeMatch) {
        return __awaiter(this, void 0, void 0, function () {
            var startIndex, res, rows, totalResults, nextPage;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        startIndex = params.hasOwnProperty('start-index') ? params['start-index'] || 0 : 0;
                        routeMatch.log('Getting Analytics page offset by:', startIndex);
                        return [4 /*yield*/, analytics.data.ga.get(params, options)];
                    case 1:
                        res = _a.sent();
                        if (!res || !res.data || !res.data.rows)
                            return [2 /*return*/, []];
                        rows = res.data.rows || [];
                        totalResults = res.data.totalResults || 0;
                        if (!(startIndex + rows.length < totalResults)) return [3 /*break*/, 3];
                        params['start-index'] = startIndex + rows.length;
                        return [4 /*yield*/, this.getData(params, options, routeMatch)];
                    case 2:
                        nextPage = _a.sent();
                        rows.push.apply(rows, nextPage);
                        _a.label = 3;
                    case 3: return [2 /*return*/, rows];
                }
            });
        });
    };
    return TaskGAGet;
}(task_base_1.TaskBase));
exports.TaskGAGet = TaskGAGet;
/* tslint:enable:no-any */ 
//# sourceMappingURL=task-ga-get.js.map